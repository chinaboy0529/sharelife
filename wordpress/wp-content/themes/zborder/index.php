...
if (is_home () || is_front_page ()) {
	...
	<?php if (is_search()) : ?>
		<div class="post_path">
			...
			href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php _e('Home', 'zborder'); ?></a> &raquo; <?php echo sprintf(__('<h1>Search Results for %s</h1>','zborder'), get_search_query()); ?>
		</div>
	<div class="archive_title">
...
if ($paged < 2 && is_sticky ()) {
			echo '<span class="sticky_mark">' . __ ( 'Sticky post!', 'zborder' ) . '</span>';
		}
		?>
				<a href="<?php the_permalink(); ?>"
				title="<?php echo esc_attr( get_the_time() ); ?>" rel="bookmark"><?php echo get_the_date(); ?></a>
			| <a
